...
class ResNet34s8Network(base.NeuralNetwork):
    ...
    def __init__(self, context, loss, isTraining):
        ...
        self.loss = loss
        # Base init
        super(ResNet34s8Network, self).__init__(context, isTraining)
    ...
    def initializeInputs(self, context, isTraining):
        ...
        if isTraining:
            ...
            self.featureMapInput = Input(shape=(None, None, 128))
    ...
    def initializeModels(self, context, isTraining):
        ...
        if not weights_path:
            weights_path = context.getConfig('NeuralNetwork', 'resnet_34_weights_path')
    ...
    def classifier_layers(self, x, input_shape, trainable=False):
        ...
        x = self.conv_block_td(x, 3, [256, 256], stage=4, block='a',strides=(2,2), input_shape=input_shape, trainable = trainable)
        x = self.identity_block_td(x, 3, [256, 256], stage=4, block='b', trainable = trainable)
        x = self.identity_block_td(x, 3, [256, 256], stage=4, block='c', trainable = trainable)
        x = self.identity_block_td(x, 3, [256, 256], stage=4, block='d', trainable = trainable)
        x = self.identity_block_td(x, 3, [256, 256], stage=4, block='e', trainable = trainable)
        x = self.identity_block_td(x, 3, [256, 256], stage=4, block='f', trainable = trainable)
