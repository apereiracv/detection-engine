...
class ResNet34Network(base.NeuralNetwork):
    """ Implementation of ResNet 34 neural network
    """
    ...
    def __init__(self, context, loss, isTraining):
        ...
        self.loss = loss
        # Base init
        super(ResNet34Network, self).__init__(context, isTraining)
    ...
    def initializeInputs(self, context, isTraining):
        ...
        if isTraining:
            ...
            self.featureMapInput = Input(shape=(None, None, 256))
    ...
    def initializeModels(self, context, isTraining):
        ...
        # self.loadWeigths(context.getConfig('NeuralNetwork', 'resnet_weights_path'))

        # Compile model with desired optimizer and loss functions
        # TODO: Change loss class and use class count from context
        frcnnLoss = self.loss
    ...
    def createBaseLayers(self, imageInput, trainable):
        ...
        x = self.conv_block(x, 3, [128, 128], stage=3, block='a', trainable = trainable)
        x = self.identity_block(x, 3, [128, 128], stage=3, block='b', trainable = trainable)
        x = self.identity_block(x, 3, [128, 128], stage=3, block='c', trainable = trainable)
        x = self.identity_block(x, 3, [128, 128], stage=3, block='d', trainable = trainable)

        x = self.conv_block(x, 3, [256, 256], stage=4, block='a', trainable = trainable)
        x = self.identity_block(x, 3, [256, 256], stage=4, block='b', trainable = trainable)
        x = self.identity_block(x, 3, [256, 256], stage=4, block='c', trainable = trainable)
        x = self.identity_block(x, 3, [256, 256], stage=4, block='d', trainable = trainable)
        x = self.identity_block(x, 3, [256, 256], stage=4, block='e', trainable = trainable)
        x = self.identity_block(x, 3, [256, 256], stage=4, block='f', trainable = trainable)
    ...
    def conv_block(self, input_tensor, kernel_size, filters, stage, block, strides=(2, 2), trainable=True):
        ...
        shortcut = Convolution2D(nb_filter2, (kernel_size, kernel_size), padding='same', strides=strides, name=conv_name_base + '1', trainable=trainable)(input_tensor)
    ...
    def conv_block_td(self, input_tensor, kernel_size, filters, stage, block, input_shape, strides=(2, 2), trainable=True):
        ...
        shortcut = TimeDistributed(Convolution2D(nb_filter2, (kernel_size, kernel_size), padding='same', strides=strides, trainable=trainable, kernel_initializer='normal'), name=conv_name_base + '1')(input_tensor)
    ...
    def classifier_layers(self, x, input_shape, trainable=False):
        ...
        x = self.conv_block_td(x, 3, [512, 512], stage=5, block='a', input_shape=input_shape, strides=(2, 2), trainable=trainable)
        x = self.identity_block_td(x, 3, [512, 512], stage=5, block='b', trainable=trainable)
        x = self.identity_block_td(x, 3, [512, 512], stage=5, block='c', trainable=trainable)
